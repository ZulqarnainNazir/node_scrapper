...
$(document).ready(function() {
    ...
    validateStep2Submit();
    handleShippingCosts();
    handleDelete();//LÃ¶schen von EintrÃ¤gen im Schritt 4
    validateDiscountCode();//Wird nicht mehr gebraucht
    handleAddress();//AdressÃ¤nderung im Confirm-Prozess
    handleTabs();
    filterJournal(); //Filtern von JournaleintrÃ¤gen im Partnerbereich
    handleAccordion(); //FAQ
    menu($("ul.menu li"));
    handleParentClick();
    styleButtons();
...
function validateStep4Submit() {
    ...
    //PrÃ¼fen ob das Rabatt-Code gÃ¼ltig ist
    if(discount.val() != '')
    {
        var data = {
            M: 'dwt',
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                if(!data)
                {
                    ...
                    discount.attr('title', 'Dieser Rabattcode ist leider ungÃ¼ltig. ');
                    status = false;
                }
                else {
                    discount.css('border', '');
    ...
    if (!status)
    {
        alert('Bitte geben sie in den rot markierten Felder gÃ¼ltige Daten ein. ');
    }
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    else
                    {
                        ...
                        $('td.discount.result').empty();
                        $('input.discount.code').val('');
                        $('td.discount.result').text('UngÃ¼ltiger Code');
                    }
                }
...
/**
 * LÃ¶schen von EintrÃ¤gen im Schritt overview
 * und bei der BestÃ¤tigung
 */
function handleDelete()
{
    var tbody = $('tbody tr');
    $('td div.delete.order').click(function() {
        var status = window.confirm('Wollen Sie den Artikel wirklich lÃ¶schen?');
        if (status)
        {
...
//AddressÃ¤nderungen bei der BestÃ¤tigung
...
function validateStep8Submit() {
    ...
    {
        alert('Bitte geben Sie eine vollstÃ¤ndige Anschrift an');
        return false;
    }
    enableLoadingScreen();
    return true;
...
function filterJournal()
{
    ...
    if ($filter != undefined && $filter.length > 0) {
        ...
        if ($freitext.keyup(function(e) {
            ...
            {
                //Beim LÃ¶schen wird immer getriggert
                $freitext.trigger('change');
            }
        }))
            ;
...
//BenÃ¶tigen die Raphael.js Scripte (werden im Header geladen
 ...
 * Es kÃ¶nnen am overlay div mehrere Steuerelemente vorhanden sein:
 *	autoplay: Slideshow startet automatisch.
 *	autopause: Slideshow hÃ¤lt automatisch an wenn die Maus sich darÃ¼ber bewegt.
...
/**
 * Berechnet alles neu, sobald man eine Versandart wÃ¤hlt
 */
function handleShippingCosts()
{
    ...
    $shipping.change(function()
    {
        ...
        // Das sind die Werte der vorher ausgewÃ¤hlten Shipping-Method
        ...
        var $shippingCosts = $('dd.shipping.number span');//Zielelement fÃ¼r die Versandkosten-Zahl
        
        // Aktuell ausgewÃ¤hlte Shipping-Costs
